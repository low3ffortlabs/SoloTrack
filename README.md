...
I have very specific and unique scenarios where I am using a GPX service, such as GPXSee, on my Windows Surface PC but, my PC does not have built in GPS. I still need location services so to enable GPS on my PC, I would have to purchase and use a dedicated GPS receiver (dongle) and hope that it works with my GPX app. To get around paying the money for a GPS dongle and to be certain that it is compatible with my GPX app, I created SoloTrack. 
...
In Solotrack's development, I used Franson GpsGate 2.6 as the virtual COM port generator. SoloTrack creates a TCP server on the Android side of things and requires that the PC listens on that server, this is why a virtual COM port is required to be set up specifically with TCP/IP.
